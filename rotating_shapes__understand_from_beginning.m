...
function shapes_to_placements(stumpers, board_width, board_height)
    disp("Started: turning shapes into placements")
    ...
    for shape_count=1:size(all_orientations,2) % for shape_count=1:28
        ...
        widen = (board_width-1) - max(all_orientations(shape_count).record_1(:,1)); % if a shape has an x co-ord at 5 then it spans the entire width of the 6 block board
        % so widen would be 5-5=0
        heighten = (board_height-1) - max(all_orientations(shape_count).record_1(:,2));
    ...
    if length(stumpers) > 0 % only if there are stumpers does this section need to run
    ...
    disp("Finished: turning shapes into placements")
    disp("")
end
...
function find_solutions()
    ...
    disp("Finished finding all solutions")
    disp("")
...
function plot_figures()
    ...
    disp("Beginning plotting solutions to figures")
    num_of_solutions = size(store_all_placements_borders, 1);
    for solution = [1:num_of_solutions]
    ...
    disp("Finished plotting solutions to figures")
    disp("")
